package chat.tox.viker.activities
...
import chat.tox.viker.adapters.ChatMessagesAdapter
import chat.tox.viker.data.State
import chat.tox.viker.theme.ThemeManager
import chat.tox.viker.utils.StringExtensions.RichString
import chat.tox.viker.utils.ViewExtensions.RichView
import chat.tox.viker.utils.{AntoxLog, Constants, KeyboardOptions, Location}
import chat.tox.viker.wrapper.{ContactKey, Message, MessageType}
import chat.tox.viker._
  ...
  override def onCreate(savedInstanceState: Bundle): Unit = {
    ...
    chatListView.addOnScrollListener(new OnScrollListener {
      ...
      override def onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int): Unit = {
        ...
        scrollDateHeader.setText(recyclerView.getAdapter().asInstanceOf[chat.tox.viker.adapters.ChatMessagesAdapter].getBubbleText(targetPos))
        // System.out.println("myh=" + fastScroller.getMyHeight() + " dy=" + dy + " p=" + proportion + " hy=" + fastScroller.getHandleY() + " targetPos=" + targetPos + "t=" + recyclerView.getAdapter().asInstanceOf[chat.tox.viker.adapters.ChatMessagesAdapter].getBubbleText(targetPos))
      }
