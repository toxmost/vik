...
import chat.tox.viker.av.CallService
import chat.tox.viker.callbacks.{AntoxOnSelfConnectionStatusCallback, ToxCallbackListener, ToxavCallbackListener}
                      ...
                      if (!State.lastFileTransferActionInTheLast(State.noBatterySavingWithActionWithinLastXSeconds)) {
                        if (!State.lastIncomingMessageActionInTheLast(State.noBatterySavingWithActionWithinLastXSeconds)) {
                          ...
                          try {
                            ...
                            intent.setAction("chat.tox.viker.TOXSERVICE_ALARM")
